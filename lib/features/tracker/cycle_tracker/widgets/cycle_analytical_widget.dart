...
class _CycleAnalyticalGraphState extends State<CycleAnalyticalGraph> {
          ...
          if (hasSelection)
            Padding(
              ...
              child: Row(
                ...
                children: [
                  _buildStatMini("STRENGTH", widget.data["strength"]?[_currentIndex], AppColors.crimson, isCompact),
                  _buildStatMini("TOTAL TONNAGE", widget.data["volume"]?[_currentIndex], Colors.orangeAccent, isCompact, isVolume: true),
                ],
              ),
            ),
  ...
  Widget _buildStatMini(String label, double? value, Color color, bool isCompact, {bool isVolume = false}) {
    String displayVal = "--";
    if (value != null) {
      displayVal = isVolume
          ? "${value >= 1 ? value.toStringAsFixed(2) : value.toStringAsFixed(3)} T"
...
class CycleComparisonWidget extends StatelessWidget {
      ...
      child: Column(
        ...
        children: [
          ...
          if (idx1 == null && idx2 == null) ...[
            ...
            SizedBox(height: isCompact ? 16.h : 12.0),
            _buildComparisonDetails(context, isCompact),
          ],
        ],
      ),
    );
  ...
  Widget _buildIntervalHeader(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          const Icon(Icons.timer_outlined, color: AppColors.crimson, size: 12.0),
  ...
  Widget _buildSinglePointView(BuildContext context, int idx) {
    final metrics = [
      ...
      {'label': "TOTAL TONNAGE", 'key': "volume", 'color': Colors.orangeAccent, 'unit': " T"},
    ];
          ...
          Padding(
            ...
            child: Row(
              ...
              children: [
                ...
                Text(
                  m['key'] == "volume"
                      ? "${val >= 1 ? val.toStringAsFixed(2) : val.toStringAsFixed(3)} T"
          ...
          GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(color: selectedIdx != null ? AppColors.crimson.withValues(alpha : 0.4) : AppColors.white.withValues(alpha : 0.05), width: 1.5),
              ),
              child: Row(
                ...
                children: [
                  ...
                  Flexible(child: Text(selectedIdx != null ? labels[selectedIdx] : "SET POINT", overflow: TextOverflow.ellipsis, style: AppTextStyles.labelSmall.adaptive(context).copyWith(color: selectedIdx != null ? Colors.white : AppColors.textSecondary.withValues(alpha : 0.4), fontWeight: FontWeight.w500))),
                ],
              ),
            ),
          ),
  ...
  Widget _buildComparisonDetails(BuildContext context, bool isCompact) {
    final List<Widget> items = [];
    ...
    for (var m in metrics) {
      ...
      if (v1 != null && v2 != null) {
        items.add(_buildMetricComparison(context, m['label'] as String, v1, v2, m['unit'] as String, m['color'] as Color, isCompact));
      }
    }
  ...
  Widget _buildMetricComparison(BuildContext context, String label, double v1, double v2, String unit, Color color, bool isCompact) {
    final delta = v2 - v1;
      ...
      child: Column(
        children: [
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              _valItem(context, "Point A", v1, unit, isCompact),
              _valItem(context, "Point B", v2, unit, isCompact),
              _valItem(context, "Difference", delta, unit, isCompact, isDelta: true),
            ],
          ),
        ],
      ),
  ...
  Widget _valItem(BuildContext context, String l, double v, String u, bool isCompact, {bool isDelta = false}) {
    return Column(
      children: [
